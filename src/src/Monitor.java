...
public class Monitor
{
	...
	public synchronized void requestSleep(final int piTID)
	{
		try
		{
			while(true) //infinite loop
			{
				...
				{
					System.out.println("Someone is talking, philosopher" +piTID + " cannot sleep.");
					wait(); //else wait for someone to finish talking
				}
			}
		}
	...
	public synchronized void requestPepper(final int piTID)
	{
		...
		{
			try {
			if (state[index]==State.EATING&&pepperShakers>0) //only if someone isn't talking, he can sleep
			{
				pepperShakers--;
				break; //exit
			}
